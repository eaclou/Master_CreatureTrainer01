...
public class Playground_Critter5B : MonoBehaviour {

	public bool reset = true;
	public string sourceFileNameMain;
	public int agentIndexMain;
	public string sourceFileNameForward;
	public int agentIndexForward;
	public string sourceFileNameStop;
	public int agentIndexStop;
	public string sourceFileNameTurn;
	public int agentIndexTurn;
	public string sourceFileNameAway;
	public int agentIndexAway;
	public int currentBehavior = 1;
	//public bool fileIsPopulation = true;
	public Transform targetTransform;
	public float masterScale = 0.5f;

	private string fileAddressRoot = "E:/Unity Projects/GitHub/ANNTrainer/CreatureTrainer/Assets/TrainingSaves/Populations/";
	...
	private Light[] LightCritterSegments;
	private Material critterDefaultMat;
	private Genome genomeMain;
	private Genome genomeStop;
	private Genome genomeForward;
	private Genome genomeTurn;
	private Genome genomeAway;
	...
	public Vector3 CritterCOM {
		get {
			return critterCOM;
		}
	}
	public Vector3 critterVelocity = new Vector3(0f, 0f, 0f);
	public Vector3 critterTargetVector = new Vector3(0f, 0f, 0f);
	...
	public float jointMotorForceMax = 800f;
	public float jointMotorSpeedMax = 400f;
	public float jointMotorForce = 0f;
	public float jointMotorSpeed = 0f;
	...
	void FixedUpdate () {
		if(critterSourceIsValid) {
			...
			Genome behaviorGenome = new Genome();
			if(currentBehavior == 1) {
				behaviorGenome = genomeMain;
			}
			else if(currentBehavior == 2) {
				behaviorGenome = genomeForward;
			}
			else if(currentBehavior == 3) {
				behaviorGenome = genomeStop;
			}
			else if(currentBehavior == 4) {
				behaviorGenome = genomeTurn;
			}
			else if(currentBehavior == 5) {
				behaviorGenome = genomeAway;
			}
			else {
				behaviorGenome = genomeMain;
			}
			sourceAgent.brain.InitializeBrainFromGenome(behaviorGenome);
	...
	private void InitializeCritter() {
		...
		bool sourceMainIsValid = false;
		bool sourceForwardIsValid = false;
		bool sourceStopIsValid = false;
		bool sourceTurnIsValid = false;
		bool sourceAwayIsValid = false;

		// MAIN:
		if(sourceFileNameMain != "") { // if field is filled in
			if(System.IO.File.Exists (fileAddressRoot + sourceFileNameMain + fileAddressExt)) {  // check if file exists
				Debug.Log (fileAddressRoot + sourceFileNameMain + fileAddressExt + " Exists!!!");
				
				Population populationToLoad = ES2.Load<Population>(fileAddressRoot + sourceFileNameMain + fileAddressExt);				
				//Debug.Log ("genomeBiases.Length: " + populationToLoad.masterAgentArray[0].genome.genomeBiases.Length.ToString());
				if(populationToLoad.masterAgentArray[agentIndexMain] != null) {
					sourceAgent = populationToLoad.masterAgentArray[agentIndexMain];
					genomeMain = populationToLoad.masterAgentArray[agentIndexStop].genome;
					...
					sourceMainIsValid = true;
				}
				else {
					Debug.Log ("Agent (Main) " + agentIndexMain + " object is NULL!");
				}
			}
			else {
				Debug.LogError("No Source File (Main) Exists!");
			}
		}
		// FORWARD:
		if(sourceFileNameForward != "") { // if field is filled in
			if(System.IO.File.Exists (fileAddressRoot + sourceFileNameForward + fileAddressExt)) {  // check if file exists
				Debug.Log (fileAddressRoot + sourceFileNameForward + fileAddressExt + " Exists!!!");
				
				Population populationToLoad = ES2.Load<Population>(fileAddressRoot + sourceFileNameForward + fileAddressExt);				
				//Debug.Log ("genomeBiases.Length: " + populationToLoad.masterAgentArray[0].genome.genomeBiases.Length.ToString());
				if(populationToLoad.masterAgentArray[agentIndexForward] != null) {
					genomeForward = populationToLoad.masterAgentArray[agentIndexForward].genome;
					sourceForwardIsValid = true;
				}
				else {
					Debug.Log ("Agent (Forward) " + agentIndexForward + " object is NULL!");
				}
			}
			else {
				Debug.LogError("No Source File (Forward) Exists!");
			}
		}
		// STOP:
		if(sourceFileNameStop != "") { // if field is filled in
			if(System.IO.File.Exists (fileAddressRoot + sourceFileNameStop + fileAddressExt)) {  // check if file exists
				Debug.Log (fileAddressRoot + sourceFileNameStop + fileAddressExt + " Exists!!!");
				
				Population populationToLoad = ES2.Load<Population>(fileAddressRoot + sourceFileNameStop + fileAddressExt);
				if(populationToLoad.masterAgentArray[agentIndexStop] != null) {
					genomeStop = populationToLoad.masterAgentArray[agentIndexStop].genome;
					sourceStopIsValid = true;
				}
				else {
					Debug.Log ("Agent (Stop) " + agentIndexStop + " object is NULL!");
				}
			}
			else {
				Debug.LogError("No Source File (Stop) Exists!");
			}
		}
		// TURN:
		if(sourceFileNameTurn != "") { // if field is filled in
			if(System.IO.File.Exists (fileAddressRoot + sourceFileNameTurn + fileAddressExt)) {  // check if file exists
				Debug.Log (fileAddressRoot + sourceFileNameTurn + fileAddressExt + " Exists!!!");
				
				Population populationToLoad = ES2.Load<Population>(fileAddressRoot + sourceFileNameTurn + fileAddressExt);
				if(populationToLoad.masterAgentArray[agentIndexTurn] != null) {
					genomeTurn = populationToLoad.masterAgentArray[agentIndexTurn].genome;
					sourceTurnIsValid = true;
				}
				else {
					Debug.Log ("Agent (Turn) " + agentIndexTurn + " object is NULL!");
				}
			}
			else {
				Debug.LogError("No Source File (Turn) Exists!");
			}
		}
		// AWAY:
		if(sourceFileNameAway != "") { // if field is filled in
			if(System.IO.File.Exists (fileAddressRoot + sourceFileNameAway + fileAddressExt)) {  // check if file exists
				Debug.Log (fileAddressRoot + sourceFileNameAway + fileAddressExt + " Exists!!!");
				
				Population populationToLoad = ES2.Load<Population>(fileAddressRoot + sourceFileNameAway + fileAddressExt);
				if(populationToLoad.masterAgentArray[agentIndexAway] != null) {
					genomeAway = populationToLoad.masterAgentArray[agentIndexAway].genome;
					sourceAwayIsValid = true;
				}
				else {
					Debug.Log ("Agent (Away) " + agentIndexAway + " object is NULL!");
				}
			}
			else {
				Debug.LogError("No Source File (Away) Exists!");
			}
		}
		if(sourceMainIsValid && sourceForwardIsValid && sourceStopIsValid && sourceTurnIsValid && sourceAwayIsValid) {
		...
		if(critterSourceIsValid) {
			sourceAgent.brain = new BrainANN_FF_Layers_A2A() as BrainANN_FF_Layers_A2A;		
			sourceAgent.brain.InitializeBrainFromGenome(genomeMain);
		}
	}
	...
	private void InitializeGameLoopModel() {
		...
		critterDefaultMat = new Material (Shader.Find("Custom/SimpleVertexColorShader"));
		//critterDefaultMat.color = Color.cyan;
	...
	private void InstantiateGamePieces() {		
		for(int i = 0; i < critterNumberOfSegments; i++) {
			...
			GOcritterSegments[i].gameObject.layer = 8;
			GOcritterSegments[i].transform.SetParent(this.gameObject.transform);
			GOcritterSegments[i].AddComponent<Playground_GamePieceWormSegment>().InitGamePiece();
			GOcritterSegments[i].AddComponent<Light>().range = 2.5f;
			ParticleSystem particles = GOcritterSegments[i].AddComponent<ParticleSystem>();
			particles.startSpeed = 0f;
			particles.emissionRate = 350f;
			particles.maxParticles = 700;
			particles.startLifetime = 0.1f;
			particles.startSize = 0.4f;


			//GOcritterSegments[i].GetComponent<ParticleEmitter>()
			//LightCritterSegments[i].type = LightType.Point;
			if(i == critterNumberOfSegments - 1) {
				GOcritterSegments[i].AddComponent<BoxCollider>().isTrigger = true;
				GOcritterSegments[i].AddComponent<Playground_CollisionTrigger>();
			//GOcritterSegments[i].AddComponent<BoxCollider>().size = new Vector3(0.001f, 0.001f, 0.001f);
			}
		...
		/*for(int j = 0; j < critterNumberOfSegments; j++) {
			for(int k = 0; k < critterNumberOfSegments; k++) {
				if(k > j) {
					Physics.IgnoreCollision(GOcritterSegments[j].GetComponent<Collider>(), GOcritterSegments[k].GetComponent<Collider>());
					//Debug.Log ("Ignoring Collision btw: " + j.ToString() + " & " + k.ToString());
				}
			}
		}*/
	}
	...
	private void DisablePhysicsGamePieceComponents() { 
		for(int w = 0; w < critterNumberOfSegments; w++) {
			//if(GOcritterSegments[w].GetComponent<BoxCollider>() != null) {
				...
				//GOcritterSegments[w].GetComponent<BoxCollider>().size = new Vector3(0.001f, 0.001f, 0.001f);
			//}
	...
	private void SetPhysicsGamePieceTransformsFromData() {
		...
		GOcritterSegments[GOcritterSegments.Length - 1].transform.localScale *= 1.5f;
		GOcritterSegments[0].transform.localScale *= 0.75f;
	}
		...
		for(int w = 0; w < critterNumberOfSegments; w++) {
			...
			if(w > 0) {  // if not the first root segment:
				...
				if(GOcritterSegments[w].GetComponent<ConfigurableJoint>() == null) {
					...
					//GOcritterSegments[w].GetComponent<ConfigurableJoint>().enablePreprocessing = false;
				}
				else {
			...
			//if(GOcritterSegments[w].GetComponent<BoxCollider>() != null) {
				...
				//GOcritterSegments[w].GetComponent<BoxCollider>().size = new Vector3(1f, 1f, 1f);
			//}
		}		
	...
	private void Tick() {  // Runs the mini-game for a single evaluation step.
		...
		for(int w = 0; w < critterNumberOfSegments; w++) {
			...
			if(GOcritterSegments[w].GetComponent<ConfigurableJoint>() != null) {
				if(critterBodyGenome.creatureBodySegmentGenomeList[w].jointType == CreatureBodySegmentGenome.JointType.HingeX) {
					...
					motorTarget = critterSegmentArray_MotorTargetX[w][0];
					...
					motorTarget = critterSegmentArray_MotorTargetY[w][0];
					GOcritterSegments[w].GetComponent<ConfigurableJoint>().targetAngularVelocity = targetVel;
				}
					...
					motorTarget = critterSegmentArray_MotorTargetZ[w][0];
					GOcritterSegments[w].GetComponent<ConfigurableJoint>().targetAngularVelocity = targetVel;
				}
			}
			...
			// COLORS!!! v0.1			
			Mesh segmentMesh = GOcritterSegments[w].GetComponent<MeshFilter>().sharedMesh;
			motorTarget *= Mathf.Pow(jointMotorSpeed / jointMotorSpeedMax, 0.25f);
			float val = Mathf.Lerp (0f, 1f, Mathf.Pow(motorTarget, 4f));
			Color vertexColor = Color.Lerp (new Color(1f, 0.2f, 0.35f), new Color(0.2f, 1f, 0.35f), motorTarget * 0.5f + 0.5f) * val;
			GOcritterSegments[w].GetComponent<Light>().color = vertexColor;
			vertexColor.a = 0.03f;
			GOcritterSegments[w].GetComponent<ParticleSystem>().startColor = vertexColor;
			//Debug.Log (vertexColor.ToString());
			//for(int c = 0; c < segmentMesh.colors.Length; c++) {
			//	segmentMesh.colors[c] = Color.red;
			//	Debug.Log (segmentMesh.colors[c].ToString());
			//}
			segmentMesh.SetVertexColor(vertexColor);
		}
		
		// Update Game Data:
		//float avgMass = critterTotalMass / critterNumberOfSegments;
		Vector3 prevCOM = critterCOM;
		critterCOM.x = 0f;
		critterCOM.y = 0f;
		critterCOM.z = 0f;
		...
		critterVelocity = critterCOM - prevCOM;
		critterTargetVector = new Vector3(targetPosX[0] - critterCOM.x, targetPosY[0] - critterCOM.y, targetPosZ[0] - critterCOM.z);
		gameTicked = true;
	}
