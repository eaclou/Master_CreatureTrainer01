...
public class BrainNetworkVisualizer : MonoBehaviour {
    ...
    public void SetNeuronSegmentPositions(BrainNEAT brain) {
        ...
        for (int i = 0; i < sourceCritter.segaddonJointMotorList.Count; i++) {
            if (sourceCritter.critterSegmentList[sourceCritter.segaddonJointMotorList[i].segmentID].GetComponent<CritterSegment>().sourceNode.jointLink.jointType == CritterJointLink.JointType.DualXY) {
                // Add an extra one if it's a dual Joint
                brain.outputNeuronList[currentOutputIndex].worldPos = sourceCritter.critterSegmentList[sourceCritter.segaddonJointMotorList[i].segmentID].transform.position;
    ...
    public Mesh BuildNewMesh(BrainNEAT brain) {
        ...
        for(int i = 0; i < brain.neuronList.Count; i++) {
            ...
            if (brain.neuronList[i].nodeType == GeneNodeNEAT.GeneNodeType.In) {
                xpos = (float)(currentInputIndex + 1) / (float)(brain.inputNeuronList.Count + 1) + (-neuronRadius / 2f) - 0.5f;
                ypos = 0f;
                ...
                xpos = (float)(currentOutputIndex + 1) / (float)(brain.outputNeuronList.Count + 1) + (-neuronRadius / 2f) - 0.5f;
                ypos = 1f - neuronRadius;
                zpos = UnityEngine.Random.Range(-0.5f, 0.5f);
                currentOutputIndex++;
                BuildNodeSphere(meshBuilder, new Vector3(xpos, ypos, zpos), size, GetColorFromNeuron(brain.neuronList[i]));
            }
        ...
        for (int h = 0; h < hiddenNodeIndicesList.Count; h++) {
            BuildNodeSphere(meshBuilder, new Vector3(nodePositionsList[hiddenNodeIndicesList[h]].x, nodePositionsList[hiddenNodeIndicesList[h]].y, nodePositionsList[hiddenNodeIndicesList[h]].z), Mathf.Min((neuronRadius * brain.neuronList[hiddenNodeIndicesList[h]].currentValue[0] * 0.2f + 0.02f), 2.0f), GetColorFromNeuron(brain.neuronList[hiddenNodeIndicesList[h]]));
        }
        ...
        for(int c = 0; c < brain.connectionList.Count; c++) {
            ...
            BuildLinkBezier(meshBuilder, connectionBezier, connectionWidth, GetColorFromConnection(brain.connectionList[c], brain));
    ...
    public void MoveNeurons(BrainNEAT brain, int numIter) {
        ...
        for (int phase = 0; phase < numIterations; phase++) {
            ...
            for (int c = 0; c < brain.connectionList.Count; c++) {
                ...
                nodePositionsList[fromNode] += linkVector * attractByConnections * dist;  // move fromNode towards toNode proportionally based on its distance
                nodePositionsList[toNode] -= linkVector * attractByConnections * dist;  // move fromNode towards toNode proportionally based on its distance  
                ...
                bezierCurveList[c].points[1] += vectorControl1ToRest * attractByConnections * dist1 * 4f;
                bezierCurveList[c].points[2] += vectorControl2ToRest * attractByConnections * dist2 * 4f;
                ...
                for (int b = 0; b < bezierCurveList.Count; b++) {
                    ...
                    if(b != c) {  // if not comparing to itself:
                        for (int control = 0; control < 4; control++) {  // go through each of the 4 points in the other bezierCurve connection:
                            ...
                            bezierCurveList[c].points[1] -= linkVector * repelFromNeurons * distance * 0.4f;
                            bezierCurveList[c].points[2] -= linkVector * repelFromNeurons * distance * 0.4f;
                        }
                    }
                }
            }
            ...
            for (int j = 0; j < nodePositionsList.Count; j++) {  // for all nodes
                for (int k = 0; k < nodePositionsList.Count; k++) {   // go through all nodes
                    if (j != k) {  // if not comparing to itself
                        ...
                        nodePositionsList[j] -= vectorToNode * repelFromNeurons / dist;
                        nodePositionsList[k] += vectorToNode * repelFromNeurons / dist;
                    }
                }
    ...
    private Color GetColorFromNeuron(NeuronNEAT neuron) {
        //black = -1, white = 1
        float val01 = neuron.previousValue * 0.5f + 0.5f;
        Color newColor = new Color(val01, val01, val01);
        if (neuron.nodeType == GeneNodeNEAT.GeneNodeType.In) {
            newColor = Color.Lerp(newColor, new Color(0f, 1f, 0f), 0f);
        }
        if (neuron.nodeType == GeneNodeNEAT.GeneNodeType.Hid) {
            newColor = Color.Lerp(newColor, new Color(1f, 1f, 0f), 0f);
        }
        if (neuron.nodeType == GeneNodeNEAT.GeneNodeType.Out) {
            newColor = Color.Lerp(newColor, new Color(1f, 0f, 0f), 0f);
        }
    ...
    private Color GetColorFromConnection(ConnectionNEAT connection, BrainNEAT brain) {
        ...
        float val01 = brain.neuronList[connection.fromNodeID].currentValue[0] * connection.weight[0] * 0.5f + 0.5f;
        Color newColor = new Color(val01, val01, val01);
    ...
    public void BuildLinkBezier(MeshBuilder meshBuilder, BezierCurve connectionBezier, float width, Color color) {
        List<int> verticesList = new List<int>();

        float m_splineStartRadius = width;
        float m_splineEndRadius = width;
    ...
    public void UpdateConnectionVertexPositionsBezier(BrainNEAT brain) {
        ...
        for (int c = 0; c < connectionVertexList.Count; c++) {
            ...
            */
            ...
            float m_splineStartRadius = Mathf.Min(brain.neuronList[brain.connectionList[c].fromNodeID].previousValue * connectionWidth, 0.05f);
            float m_splineEndRadius = Mathf.Min(brain.neuronList[brain.connectionList[c].toNodeID].previousValue * connectionWidth, 0.05f); ;
            int m_splineHeightSegmentCount = 8;
            int m_splineRadialSegmentCount = 4;
